...
export default function EditDataPage({ params }: { params: { id: string } }) {
  ...
  useEffect(() => {
    if (!isLoading && isAuthenticated) {
      // Fetch the specific data item for editing
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.ok) {
        // Redirect to data detail on success
        router.push(`/data/${id}`);
      } else {
        const data = await response.json();
        setError(data.detail || 'Failed to update data');
      }
    } catch (err) {
      ...
      <main className="py-6">
        <div className="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="pb-5 border-b border-gray-200">
            <h1 className="text-2xl font-semibold text-gray-900">Edit Data</h1>
          </div>
